...
import { ILokiObj, IContent, IExercise } from 'ost-shared';
import {
  appStateMgmt,
  IAppStateActions,
  IAppStateModel,
...
const exercisesCollection = collectionFactory<IExercise>('exercises', ['id', 'name']);
const usersCollection = collectionFactory<IContent & ILokiObj>('users', ['id', 'name']);

export interface IAppModel extends IAppStateModel, CollectionsModel<IContent & ILokiObj> {
  exercises: CollectionType<IExercise>;
  users: CollectionType<IContent & ILokiObj>;
}

export interface IActions extends IAppStateActions, CollectionsActions<IExercise> {}
